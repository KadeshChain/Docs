...
export function getRedocThemes(customTheme, customDarkTheme = customTheme) {
    return {
        lightTheme: getThemeOptions(customTheme, false),
        darkTheme: getThemeOptions(customDarkTheme, true),
    };
}
export async function getGlobalData({ primaryColor, primaryColorDark = primaryColor, theme: customThemeDeprecated, options, }) {
    let redoclyOptions;
    if (options) {
        if (typeof options === 'string') {
            redoclyOptions = (await loadConfig({
                configPath: options,
            })).theme.openapi;
        }
        else {
